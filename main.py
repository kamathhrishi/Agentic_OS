...
# Show partial key for debugging (first 8 and last 4 chars)
if HYPERSPELL_API_KEY:
    masked_key = f"{HYPERSPELL_API_KEY[:8]}...{HYPERSPELL_API_KEY[-4:]}" if len(HYPERSPELL_API_KEY) > 12 else "***"
    print(f"🔑 HYPERSPELL_API_KEY loaded: {masked_key} (length: {len(HYPERSPELL_API_KEY)})")
else:
    print("⚠️  HYPERSPELL_API_KEY is NOT set in .env")
...
- The "data" field is always required and must be an object (empty {} for conversational messages)"""
    ...
    try:
        ...
        print(f"🔍 Checking hyperspell for message: '{user_message[:50]}...'")
        print(f"🔍 HYPERSPELL_ENABLED: {HYPERSPELL_ENABLED}")
        hyperspell_sources = detect_hyperspell_sources(user_message, history_messages)
        print(f"🔍 Detected hyperspell sources: {hyperspell_sources}")
        if hyperspell_sources:
            print(f"🔍 Fetching hyperspell context for sources: {hyperspell_sources}")
            ...
            if hyperspell_context_memories:
                print(f"✅ Retrieved {len(hyperspell_context_memories)} Hyperspell memories!")
                logger.info(
                ...
                print(f"⚠️  No hyperspell memories found for sources: {hyperspell_sources}")
        else:
            print(f"⚠️  No hyperspell sources detected in message")
